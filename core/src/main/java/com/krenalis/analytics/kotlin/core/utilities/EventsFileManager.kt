 ...
 * sent as-is to the Krenalis batch upload endpoint.
 *
 * Some terms:
...
class EventsFileManager(
    ...
    private val fileIndexKey = if(subject == null) "krenalis.events.file.index.$writeKey" else "krenalis.events.file.index.$writeKey.$subject"
    ...
    private fun finish() {
        ...
        val contents = """],"sentAt":"${KrenalisInstant.now()}","writeKey":"$writeKey"}"""
        writeToFile(contents.toByteArray(), file)
