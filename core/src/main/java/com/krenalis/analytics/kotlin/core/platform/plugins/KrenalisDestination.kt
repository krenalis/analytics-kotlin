...
import com.krenalis.analytics.kotlin.core.platform.EventPipeline
import com.krenalis.analytics.kotlin.core.platform.Plugin
import com.krenalis.analytics.kotlin.core.platform.VersionedPlugin
import com.krenalis.analytics.kotlin.core.platform.policies.CountBasedFlushPolicy
import com.krenalis.analytics.kotlin.core.platform.policies.FlushPolicy
import com.krenalis.analytics.kotlin.core.platform.policies.FrequencyFlushPolicy
import kotlinx.coroutines.launch
import kotlinx.serialization.Serializable
import sovran.kotlin.Subscriber

@Serializable
data class KrenalisSettings(
    var apiKey: String,
    var endpoint: String? = null
)

/**
 * Krenalis Analytics plugin that is used to send events to Krenalis's tracking api, in the choice of region.
 * How it works
 ...
 */
class KrenalisDestination: DestinationPlugin(), VersionedPlugin, Subscriber {

    private var pipeline: EventPipeline? = null
    var flushPolicies: List<FlushPolicy> = emptyList()
    override val key: String = "Krenalis"
    ...
    override fun setup(analytics: Analytics) {
        ...
        with(analytics) {
            ...
            analyticsScope.launch(analyticsDispatcher) {
                store.subscribe(
                    subscriber = this@KrenalisDestination,
                    stateClazz = System::class,
                    initialState = true,
                    handler = this@KrenalisDestination::onEnableToggled
                )
            }
        }
    }
    ...
    override fun update(settings: Settings, type: Plugin.UpdateType) {
        ...
        if (settings.hasIntegrationSettings(this)) {
            ...
            settings.destinationSettings<KrenalisSettings>(key)?.endpoint?.let {
