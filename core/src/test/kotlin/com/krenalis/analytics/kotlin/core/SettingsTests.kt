...
class SettingsTests {
    ...
    fun `checkSettings updates settings`() = runTest {
        ...
        assertEquals(true, curSettings?.hasIntegrationSettings("Krenalis"))
    }
    ...
    fun `plugin added before settings is available updates plugin correctly`() = runTest {
        ...
        analytics = testAnalytics(Configuration(
            writeKey = "123",
            application = "Test",
            autoAddKrenalisDestination = false
        ), testScope, testDispatcher)
        val mockPlugin = spyk<StubPlugin>()
    ...
    fun `fetchSettings returns null when Settings string is invalid`() {
        ...
        mockHTTPClient("")
        var settings = analytics.fetchSettings("foo", "test.example.com/v1")
        assertNull(settings)

        // Null on invalid JSON
        mockHTTPClient("hello")
        settings = analytics.fetchSettings("foo", "test.example.com/v1")
        assertNull(settings)
