...
internal class KrenalisLogTest {

    @Test
    fun `can call krenalisLog() `() {
        ...
        Analytics.krenalisLog("test")

        assertTrue(parseLogCalled.get())
    }

    @Test
    fun `can call krenalisLog() with different log filter kind`() {
        ...
        Analytics.krenalisLog("test") // Default LogFilterKind is ERROR
        Analytics.krenalisLog("test", kind = LogKind.WARNING)
        Analytics.krenalisLog("test", kind = LogKind.DEBUG)
